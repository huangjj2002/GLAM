...
class BalanceSampler(Sampler):
    ...
    def __init__(self, labels, ratio=8):
        self.r = ratio - 1
    ...
    def __iter__(self):
        ...
        pos_index = np.random.choice(pos_index, self.neg_length // self.r).reshape(
            -1, 1
        )
...
class BalanceSamplerV2(Sampler):
    def __init__(
        ...
        if start_ratio != end_ratio:
            ...
            self.ratios = np.concatenate(
                [self.ratios, np.full((num_epochs - num_sched_epochs,), end_ratio)]
            )
        else:
    ...
    def set_epoch(self, ep):
        assert (
            ep < self.num_epochs
        ), f"WARNING: Invalid set_epoch() with ep={ep} while max_epoch={self.num_epochs}"
    ...
    def _pre_compute_epoch_idxs(self, ratio, one_pos_mode=True):
        ...
        if min_pos_per_iter < 1:
            if one_pos_mode:
                ...
                print(
                    f"WARNING: At least one batch which has no positive sample: {epoch_num_pos}, {epoch_num_iters}.\n"
                )
        ...
        for i, j in enumerate(
            np.random.choice(
                np.arange(0, epoch_num_iters, 1), num_pos_remain, replace=False
            )
        ):
        ...
        assert (
            ret_idxs.shape[0] == epoch_num_iters
            and ret_idxs.shape[1] == self.batch_size
        )
    ...
    def __iter__(self):
        ...
        print(
            f"{len(epoch_idxs) // self.batch_size} iters with {len(epoch_idxs)} samples"
        )
